...
const Vin = () => {
  ...
  {
    let picture;
    await getPicture(make, model, year).then( (result) => {
      if (result.data!==null) {
        picture=result.data;
      }
    }).catch( (e) => {
  ...
  async function getVinData() {
    ...
    await getShortVin(vin)
      .then( async (result) => {
	    ...
	    const img = await getVehiclePicture(result.data.Make.trim(), result.data.Model.trim(), result.data.ModelYear.trim());
  ...
  return (
	...
	<form onSubmit={submitHandler} className="addVin">
	  <div className="form-group input-group mt-10">
	    ...
	    <input className="form-control ml-0 mr-5" id="vin" type="text" name="vin" placeholder="Vin..." required />
	   <button className="btn-primary px-10" type="submit" name="submitBtn" >Query Vin</button>
	  </div>
	</form>
	</div>
